...
def Borrar_todos(driver,wait, modulo, submodulo, filtro=None):
    if filtro is None:
    ...
    #Borrar filtro de ubigeo
        ...
        existe_filtro_departamento = acciones_unitarias.probar_ids(driver,id_frm_departamento)

        if existe_filtro_departamento is None:
            print("El ID del filtro del departamento no fue encontrado")
        else:
            acciones_unitarias.MoverClick(driver,existe_filtro_departamento)
            sin_departamento = wait.until(EC.presence_of_element_located((By.ID, id_sin_departamento)))
            acciones_unitarias.MoverClick(driver,sin_departamento)
    ...
    #Borrar el filtro del código
        ...
        existe_filtro_codigo = acciones_unitarias.probar_ids(driver,id_frm_Codigo)
    ...
    #Borrar el estado de la DNA
        ...
        existe_estado = acciones_unitarias.probar_ids(driver,id_frm_estado)
        if existe_estado is None:
            print("El ID del filtro de estado DNA no fue encontrado")
        else:
            acciones_unitarias.MoverClick(driver,existe_estado)
            sin_estado = wait.until(EC.presence_of_element_located((By.ID, id_sin_estado)))
            acciones_unitarias.MoverClick(driver,sin_estado)
    ...
    #Borrar la fecha de supervision
        ...
        existe_fecha_desde = acciones_unitarias.probar_ids(driver,id_frm_fecha_desde)
        existe_fecha_hasta = acciones_unitarias.probar_ids(driver,id_frm_fecha_hasta)

        if existe_fecha_desde is None:
            print("El ID del filtro de fecha 'desde' no fue encontrado")
        else:
            acciones_unitarias.MoverClick(driver,existe_fecha_desde)
        ...
        if existe_fecha_hasta is None:
            ...
            acciones_unitarias.MoverClick(driver,existe_fecha_hasta)
    ...
    #Borrar el supervisor
        ...
        existe_supervisor = acciones_unitarias.probar_ids(driver,id_frm_supervisor)
        if existe_supervisor is None:
            print("El ID del supervisor no fue encontrado")
        else:
            acciones_unitarias.MoverClick(driver,existe_supervisor)
            sin_supervisor = wait.until(EC.presence_of_element_located((By.ID, id_sin_supervisor)))
            acciones_unitarias.MoverClick(driver,sin_supervisor)
            print("Filtro del supervisor limpio")



def Borrar_filtro(driver,wait, modulo, submodulo, nombre_filtro, tipo_filtro):
    print(f"Se borró el filtro {nombre_filtro} con éxito")